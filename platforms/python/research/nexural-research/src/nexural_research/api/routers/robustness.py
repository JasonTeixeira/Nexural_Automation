...
from nexural_research.api.compat import (
    adapt_block_bootstrap,
    adapt_deflated_sharpe,
    adapt_monte_carlo,
    adapt_regime,
    adapt_walk_forward,
)
...
def get_monte_carlo(
    ...
    n: int = Query(default=1000, ge=10, le=100000),
...
def get_parametric_mc(
    session_id: str = Query(default="default"),
    n_simulations: int = Query(default=5000, ge=10, le=100000),
...
def get_block_bootstrap(
    session_id: str = Query(default="default"),
    n_simulations: int = Query(default=2000, ge=10, le=50000),
    block_size: int | None = Query(default=None),
    seed: int = Query(default=42),
    periods_per_year: float | None = Query(default=None, gt=0),
):
...
def get_walk_forward(session_id: str = Query(default="default"), split: float = Query(default=0.7)):
    ...
    raw["walk_forward_efficiency"] = raw.get("out_sample_net_profit", 0) / max(
        abs(raw.get("in_sample_net_profit", 1)), 0.01
    )
    raw["folds"] = []
    in_sample = (
        f"IS: ${raw.get('in_sample_net_profit', 0):,.2f} "
        f"({raw.get('in_sample_n', 0)} trades)"
    )
    out_sample = (
        f"OOS: ${raw.get('out_sample_net_profit', 0):,.2f} "
        f"({raw.get('out_sample_n', 0)} trades)"
    )
    raw["interpretation"] = f"{in_sample} -> {out_sample}"
    return raw
...
def get_rolling_wf(
    session_id: str = Query(default="default"),
    n_windows: int = Query(default=5, ge=2, le=50),
    in_sample_pct: float = Query(default=0.7, ge=0.1, le=0.95),
...
def get_deflated_sharpe(
    session_id: str = Query(default="default"),
    n_trials: int = Query(default=100, ge=1, le=100000),
    periods_per_year: float | None = Query(default=None, gt=0),
):
    """Deflated Sharpe Ratio for overfitting detection."""
    raw = safe_serialize(
        deflated_sharpe_ratio(
            get_trades(session_id), n_trials=n_trials, periods_per_year=periods_per_year
        )
    )
...
def get_regime(
    session_id: str = Query(default="default"),
    n_regimes: int = Query(default=3, ge=2, le=10),
    window: int = Query(default=20, ge=5, le=200),
):
    """Volatility regime detection & performance analysis."""
    raw = safe_serialize(
        regime_analysis(get_trades(session_id), n_regimes=n_regimes, window=window)
    )
