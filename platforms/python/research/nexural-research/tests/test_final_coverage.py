...
def client():
    c = TestClient(app)
    csv = (
        "trade_id,symbol,side,entry_time,exit_time,net_pnl,commission,strategy,mae,mfe\n"
        + "\n".join(
    ...
    c.post(
        "/api/upload?session_id=final_cov",
        files={"file": ("final.csv", io.BytesIO(csv.encode()), "text/csv")},
    )
    return c
...
class TestAIRouterMocked:
    def test_analyze_mocked(self, client):
        """AI routes resolve sessions through the ownership-aware service."""
        mocked = {
            "kind": "trades",
            "df": pd.DataFrame(
                {
                    ...
                    "exit_time": [
                        pd.Timestamp("2025-01-01 00:15"),
                        pd.Timestamp("2025-01-02 00:15"),
                    ],
                    "instrument": ["NQ", "NQ"],
                    "strategy": ["T", "T"],
                }
        ...
        with patch("nexural_research.api.routers.ai.get_session", return_value=mocked):
            response = client.post("/api/ai/context-preview?session_id=mocked")
        assert response.status_code == 200
        assert response.json()["approx_tokens"] > 0
...
class TestExecutionQualityFull:
    def test_execution_quality_endpoint(self, client):
        ...
        client.post(
            "/api/upload?session_id=exec_full",
            files={"file": ("exec.csv", io.BytesIO(csv.encode()), "text/csv")},
        )
...
class TestRobustnessWFFallback:
    def test_wf_with_entry_time_only(self):
        ...
        result = walk_forward_split(
            df, split=0.7, ts_col="exit_time"
        )  # exit_time missing, should fallback
