...
def client():
    ...
    c.post(
        "/api/upload?session_id=cov_test",
        files={"file": ("cov.csv", io.BytesIO(csv.encode()), "text/csv")},
    )
    return c
...
class TestSessionEdgeCases:
    def test_get_trades_wrong_kind(self, client):
        ...
        client.post(
            "/api/upload?session_id=exec_cov",
        ...
        df = pd.DataFrame(
            {
                ...
                "exit_time": pd.Timestamp("2025-01-01 00:15"),
            }
        )
...
class TestUploadParsing:
    def test_upload_uses_opaque_server_id(self, client):
...
class TestComparisonEdges:
    def test_three_strategies(self):
        ...
        df_a = pd.DataFrame(
            {
                "profit": [100, -30] * 10,
                "entry_time": pd.date_range("2025-01-01", periods=20, freq="h"),
                "exit_time": pd.date_range("2025-01-01 00:15", periods=20, freq="h"),
                "instrument": "NQ",
                "strategy": "A",
            }
        )
        ...
        result = compare_strategies(
            [("a", "a.csv", df_a), ("b", "b.csv", df_b), ("c", "c.csv", df_c)]
        )
